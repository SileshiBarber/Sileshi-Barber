...
import { getReport } from "./metrics.js";
...
function buildHtml(r: ReturnType<typeof getReport>): string {
  const rows = r.topEvents
    .map(
      ({ event, count }) => `
      <tr>
        <td style="padding:8px 12px;border-bottom:1px solid #1f2833;color:#c5c6c7;font-size:13px;">
          ${event}
        </td>
        <td style="padding:8px 12px;border-bottom:1px solid #1f2833;color:#c5a059;font-size:13px;text-align:right;font-weight:700;">
          ${count}
        </td>
      </tr>`,
    )
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#0b0c10;padding:40px 0;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#1f2833;border-radius:12px;overflow:hidden;border:1px solid rgba(197,160,89,0.2);">
        ...
        <tr>
          <td style="padding:24px 36px 0;">
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="background:#0b0c10;border-radius:8px;border:1px solid rgba(255,255,255,0.05);overflow:hidden;">
              ...
              ${rows || '<tr><td colspan="2" style="padding:16px;text-align:center;color:#c5c6c7;font-size:13px;">No interactions tracked yet</td></tr>'}
            </table>
          </td>
        </tr>
        ...
        <!-- Footer -->
        <tr>
          <td style="padding:28px 36px 24px;border-top:1px solid rgba(255,255,255,0.05);margin-top:24px;">
            ...
            <p style="margin:4px 0 0;color:#c5c6c7;font-size:12px;opacity:0.4;">
              This report was auto-generated on ${new Date().toLocaleDateString("en-AU")}.
            </p>
          </td>
        </tr>
...
async function sendMonthlyReport(): Promise<void> {
  ...
  if (!SMTP_HOST || !SMTP_PORT || !SMTP_USER || !SMTP_PASS) {
    logger.warn("SMTP credentials not configured — skipping monthly email report");
    return;
  }
  ...
  await transporter.sendMail({
    ...
    subject: `📊 Monthly Report — Sileshi Barbershop · ${new Date().toLocaleDateString("en-AU", { month: "long", year: "numeric" })}`,
    html: buildHtml(report),
  });

  logger.info("Monthly analytics email sent to david@tykieautomation.com");
}

export function startReporter(): void {
  // Run at 9:00 AM on the 1st of every month (Melbourne time)
  cron.schedule(
    "0 9 1 * *",
    () => {
      logger.info("Cron: firing monthly analytics report");
      sendMonthlyReport().catch((err) =>
        logger.error({ err }, "Monthly report send failed"),
      );
    },
    { timezone: "Australia/Melbourne" },
  );

  logger.info("Reporter scheduled: monthly email on the 1st at 9 AM Melbourne time");
}
